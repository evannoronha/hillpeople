  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        message.classList.remove('hidden', 'text-red-700', 'dark:text-red-400');
        message.classList.add('text-green-700', 'dark:text-green-400');
        form.reset();
        submitButton.textContent = 'Subscribed!';
      } else {
        const errorData = await res.json().catch(() => ({}));
        message.textContent = errorData?.error?.message || 'Something went wrong. Please try again.';
        message.classList.remove('hidden', 'text-green-700', 'dark:text-green-400');
        message.classList.add('text-red-700', 'dark:text-red-400');
